...
- Исторически беднее по фичам: слабее работа с JSON, оконными функциями, типами, расширяемостью (разрыв сократился в 8.x).
- Нюансы дефолтов (часовые пояса, collation, strict mode); кластеризованный PK влияет на дизайн (см. postgres).
